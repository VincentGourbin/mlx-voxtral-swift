...
class SpeechDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.isdir(root):
            ...
            # Try torchaudio dataset
            try:
                ds = torchaudio.datasets.LIBRISPEECH(root, url=split, download=True)
                self.files = [ds._walker[i][0] for i in range(len(ds._walker))]
                self._torchaudio_ds = ds
            except Exception as e:
                raise ValueError(f"Cannot load dataset from {root}: {e}")
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        if hasattr(self, '_torchaudio_ds'):
...
def create_dataloader(
    ...
    def collate_fn(batch):
        ...
        # Round up to multiple of 240 * 8 (encoder total downsample)
        frame_size = 240 * 8
